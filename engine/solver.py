...
class Trainer(object):
    def __init__(self, config, args, model, dataloader, logger=None):
        ...
        self.train_num_steps_diff = config['solver']['max_epochs_diff']
        self.train_num_steps_classifier = config['solver']['max_epochs_classifier']
    ...
    def train(self):
        ...
        with tqdm(initial=step, total=self.train_num_steps_diff) as pbar:
            while step < self.train_num_steps_diff:
                total_loss = 0.
    ...
    def train_classfier(self, classifier):
        ...
        with tqdm(initial=step, total=self.train_num_steps_classifier) as pbar:
            while step < self.train_num_steps_classifier:
                total_loss = 0.
                for _ in range(self.gradient_accumulate_every):
                    x, y = next(dataloader)
