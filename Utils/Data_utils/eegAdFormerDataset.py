...
class EEGDataset(Dataset):
    ...
    def read_data(self, folderpath, length):
        """
        ...
        featuresPath = os.path.join(folderpath,"Feature")
        labelPath = os.path.join(folderpath,"Label/label.npy")
        ...
        #print(labels)
        for i in labels:
            features = np.load(os.path.join(featuresPath,f"feature_{int(i[1]):02d}.npy"))
            if(i[0] == 0):
                data_0.append(features)
            elif(i[0] == 1):
                data_1.append(features)

        # print(f"Class 0: {data_0.shape}, Class 1: {data_1.shape}")
        data_0 = np.concatenate(data_0,axis=0)
        data_1 = np.concatenate(data_1,axis=0)
        #data_2 = np.concatenate(data_2,axis=0)

        #data_0 = split_eeg_segments(data_0)
        #data_1 = split_eeg_segments(data_1)
        #data_2 = split_eeg_segments(data_2)
